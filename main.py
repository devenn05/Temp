...
SMC_V2_SETTINGS = {
    "mode_name": "SMC Smart Engine Final V2 - Optimized",
    "entry_timeframe": '15m',
    "score_threshold": 100,
    "risk": {
        "min_rr": 1.5,
        ...
        "atr_buffer_multiplier": 1.4, # Widen from 0.8 to give trades breathing room
        "partial_tp_rr": 1.5
    }
}
...
# This version is now clean and has the correct signature
def get_confluence_score_optimized(entry_df: pd.DataFrame, current_index: int, trade_type: str, settings: Dict) -> Dict:
    # Reverting to the simple, effective two-filter confirmation
...
def run_analysis_for_backtest(entry_df: pd.DataFrame, ltf_df: Optional[pd.DataFrame], trade_type: str,
                              settings: Dict[str, Any], i: int):
    ...
    if (trade_type == 'long' and not (is_4h_bias_long and ema_50_1h_slope > 0)) or \
            (trade_type == 'short' and not (is_4h_bias_short and ema_50_1h_slope < 0)): return default_response
    ...
    final_report = get_confluence_score_optimized(entry_df, i, trade_type, settings)  # Correct call
    ...
    try:
        ...
        trade_plan = {'entry_price': entry_price, 'stop_loss': sl, 'take_profit_1': tp1, 'take_profit_final': tp,
